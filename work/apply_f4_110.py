...
def force_pagebreak_before(p):
    ppr=p.find(f'{{{W}}}pPr')
    if ppr is None:
        ppr=etree.Element(f'{{{W}}}pPr'); p.insert(0,ppr)
    el=ppr.find(f'{{{W}}}pageBreakBefore')
    if el is None:
        el=etree.SubElement(ppr,f'{{{W}}}pageBreakBefore')
    el.set(f'{{{W}}}val','1')

def pagebreak_before_on(p):
    ppr=p.find(f'{{{W}}}pPr')
    if ppr is None:return False
    el=ppr.find(f'{{{W}}}pageBreakBefore')
    return el is not None and el.get(f'{{{W}}}val','1') not in ('0','false','off')
...
def complete(path:Path):
    ...
    bib=[p for p in ps if h.norm(h.txt(p))==h.norm(BIB) and h.spec(p)['book']>0]
    ...
    fi=ps.index(finals[0]); fu=ps.index(future[0]); bi=ps.index(bib[0])
    return fi < fu < bi and pagination_explicitly_off(future[0]) and pagebreak_before_on(bib[0])
...
def apply(src:Path,out:Path):
    ...
    with ZipFile(src,'r') as zin:
        ...
        # Layout-only repair: keep the future-research paragraph in Sonuç and explicitly start bibliography on a new page.
        force_pagebreak_before(pb)
    ...
    return [('F4-110','Sonuc_end','APPLIED_FINAL_JUDGMENT_FUTURE_RESEARCH_AND_BIBLIOGRAPHY_PAGE_SEPARATION')]
