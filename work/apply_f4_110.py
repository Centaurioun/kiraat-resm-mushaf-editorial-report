...
def apply(src:Path,out:Path):
    ...
    with ZipFile(src,'r') as zin:
        ...
        # normalize possible residual whitespace only by preserving runs/protected structure; source paragraph is plain.
        ...
        # Reuse nearest blank paragraph immediately before bibliography when safe.
        prev=ps[ib-1]
        if h.norm(h.txt(prev))=='' and not any(h.spec(prev)[k] for k in ['fn','instr','fld','hyper','rtl','book']):
            ...
            pfinal=new_para_like(pc,FINAL); body.insert(body.index(pb),pfinal)
        # Insert future-research paragraph after final judgment and before bibliography.
        pnew=new_para_like(pc,FUTURE)
    ...
    return [('F4-110','Sonuc_end','APPLIED_FINAL_JUDGMENT_AND_FUTURE_RESEARCH_SEPARATION')]
