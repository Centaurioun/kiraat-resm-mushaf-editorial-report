...
EXPECTED_INPUT_SHA='bff8720ab193200c649ef68856a648d74d82d6d40974b9022e3cebd6f5c2d61c'
OLD='Yazım örnekleri yalnız şekil bakımından değil, bağdaştıkları veya dışladıkları kırâat ihtimalleri açısından ele alınmaktadır.'
NEW='Yazım örnekleri, şekil özellikleriyle birlikte bağdaştıkları veya dışladıkları kırâat ihtimalleri açısından ele alınmaktadır.'
...
def apply(src,out):
    ...
    with ZipFile(src,'r') as zin:
        ...
        if satisfied(d):
            ...
            print(f'F5-011\tALREADY_SATISFIED\tP{idx}'); return
        ...
        after=text(p)
        if after!=expected or sig(p)!=bs: raise RuntimeError('F5-011 postcondition/structure failure')
        if not satisfied(d): raise RuntimeError('F5-011 document satisfaction failure')
    ...
    print(f'F5-011\tAPPLIED\tP{idx}'); print('BEFORE\t'+before); print('AFTER\t'+after)
...
if __name__=='__main__':
    if len(sys.argv)!=3: raise SystemExit('usage: apply_f5_011.py INPUT.docx OUTPUT.docx')
